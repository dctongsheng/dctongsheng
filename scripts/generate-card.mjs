...
function cardSvg(card, x, y, cardW, cardH) {
  ...
  return `
  <g>
    <rect x="${x}" y="${y}" width="${cardW}" height="${cardH}" rx="10" fill="${theme.surface}" stroke="${theme.border}"/>
    <text x="${x + 16}" y="${y + 27}" font-size="12" fill="${theme.muted}">${esc(card.label)}</text>
    <text x="${x + cardW - 14}" y="${y + 27}" font-size="11" fill="${theme.subtle}" text-anchor="end">${esc(card.d)}</text>
...
function pillRow(pad) {
  ...
  let x = pad;
  let out = `<rect x="${x}" y="14" width="248" height="26" rx="13" fill="${theme.surface}" stroke="${theme.border}"/>`;

  for (const item of items) {
    const w = item.length * 12 + 16;
    const active = item === '7D';
    if (active) out += `<rect x="${x + 5}" y="17" width="${w}" height="20" rx="10" fill="${theme.accentSoft}" stroke="${theme.accent}" stroke-opacity="0.35"/>`;
    out += `<text x="${x + 5 + w / 2}" y="31" font-size="11" text-anchor="middle" fill="${active ? theme.accent : theme.muted}" font-weight="${active ? 700 : 500}">${esc(item)}</text>`;
    x += w + 4;
  }
  ...
  for (const filter of ['工具 全部', '模型 全部', '项目 全部', '终端 全部']) {
    ...
    out += `<rect x="${x}" y="14" width="${w}" height="26" rx="13" fill="${theme.surface}" stroke="${theme.border}"/>
      <text x="${x + w / 2}" y="31" font-size="11" text-anchor="middle" fill="${theme.muted}" font-weight="500">${esc(filter)} ▾</text>`;
    x += w + 8;
  }
...
function trendPanel({ x, y, w, h, days }) {
  ...
  <g>
    ...
    <text x="${x + 18}" y="${y + 30}" font-size="13" fill="${theme.text}" font-weight="700">每日趋势</text>
    <g font-size="10" fill="${theme.muted}">
      <circle cx="${x + w - 218}" cy="${y + 26}" r="4" fill="${theme.accent}"/><text x="${x + w - 210}" y="${y + 30}">输出</text>
      <circle cx="${x + w - 178}" cy="${y + 26}" r="4" fill="${theme.inputDot}"/><text x="${x + w - 170}" y="${y + 30}">输入</text>
      <circle cx="${x + w - 138}" cy="${y + 26}" r="4" fill="${theme.cacheDot}"/><text x="${x + w - 130}" y="${y + 30}">缓存</text>
    </g>
    <rect x="${x + w - 100}" y="${y + 15}" width="84" height="22" rx="11" fill="${theme.canvas}" stroke="${theme.border}"/>
    <rect x="${x + w - 98}" y="${y + 17}" width="34" height="18" rx="9" fill="${theme.accentSoft}" stroke="${theme.accent}" stroke-opacity="0.35"/>
    <text x="${x + w - 81}" y="${y + 30}" font-size="10" text-anchor="middle" fill="${theme.accent}" font-weight="700">Token</text>
    <text x="${x + w - 48}" y="${y + 30}" font-size="10" text-anchor="middle" fill="${theme.muted}">费用</text>
    <text x="${x + 40}" y="${chartY + 8}" font-size="10" fill="${theme.subtle}" text-anchor="end">${esc(fmtTok(maxV))}</text>
    <text x="${x + 40}" y="${chartY + chartH}" font-size="10" fill="${theme.subtle}" text-anchor="end">0</text>
    <line x1="${chartX}" y1="${chartY + chartH + 0.5}" x2="${chartX + chartW}" y2="${chartY + chartH + 0.5}" stroke="${theme.borderMuted}"/>
    ${bars}
    ${labels}
  </g>`;
}
...
function heatPanel({ x, y, w, h, heat }) {
  ...
  const shades = theme.heat;
  let cells = '';
  let rowLabels = '';
  let colLabels = '';

  for (let r = 0; r < rows; r += 1) {
    rowLabels += `<text x="${gridX - 10}" y="${gridY + r * step + cell * 0.75}" font-size="10" fill="${theme.muted}" text-anchor="end">${dayNames[r]}</text>`;

    for (let c = 0; c < cols; c += 1) {
      const value = heat[r][c];
      const level = value === 0 ? 0 : Math.min(5, 1 + Math.floor((value / heatMax) * 4.999));
      cells += `<rect x="${gridX + c * step}" y="${gridY + r * step}" width="${cell}" height="${cell}" rx="2.5" fill="${shades[level]}" stroke="${value === 0 ? theme.borderMuted : 'none'}" stroke-width="${value === 0 ? 0.5 : 0}"/>`;
    }
  }

  for (let c = 0; c < cols; c += 3) {
    colLabels += `<text x="${gridX + c * step + cell / 2}" y="${gridY + rows * step + 16}" font-size="10" fill="${theme.muted}" text-anchor="middle">${String(c).padStart(2, '0')}</text>`;
  }

  let legend = `<text x="${x + w - 130}" y="${y + h - 16}" font-size="10" fill="${theme.muted}">少</text>`;
  shades.forEach((shade, i) => {
    legend += `<rect x="${x + w - 116 + i * 14}" y="${y + h - 25}" width="11" height="11" rx="2.5" fill="${shade}" stroke="${i === 0 ? theme.borderMuted : 'none'}" stroke-width="${i === 0 ? 0.5 : 0}"/>`;
  });
  legend += `<text x="${x + w - 116 + shades.length * 14 + 4}" y="${y + h - 16}" font-size="10" fill="${theme.muted}">多</text>`;

  return `
  <g>
    <rect x="${x}" y="${y}" width="${w}" height="${h}" rx="12" fill="${theme.surface}" stroke="${theme.border}"/>
    <text x="${x + 18}" y="${y + 30}" font-size="13" fill="${theme.text}" font-weight="700">分时活跃</text>
    <rect x="${x + w - 100}" y="${y + 15}" width="84" height="22" rx="11" fill="${theme.canvas}" stroke="${theme.border}"/>
    <rect x="${x + w - 98}" y="${y + 17}" width="34" height="18" rx="9" fill="${theme.accentSoft}" stroke="${theme.accent}" stroke-opacity="0.35"/>
    <text x="${x + w - 81}" y="${y + 30}" font-size="10" text-anchor="middle" fill="${theme.accent}" font-weight="700">Token</text>
    <text x="${x + w - 48}" y="${y + 30}" font-size="10" text-anchor="middle" fill="${theme.muted}">费用</text>
...
export function renderSvg({ buckets = [], sessions = [], nowMs = Date.now() }) {
  ...
  const cardsRow1 = [
    { label: '预估费用', value: `$${cur.cost.toFixed(2)}`, d: delta(cur.cost, prev.cost), color: theme.success },
    { label: '总 Token', value: fmtTok(cur.total), d: delta(cur.total, prev.total), color: theme.text },
    { label: '输入 Token', value: fmtTok(cur.input), d: delta(cur.input, prev.input), color: theme.text },
    { label: '输出 Token', value: fmtTok(cur.output), d: delta(cur.output, prev.output), color: theme.text },
    { label: '缓存 Token', value: fmtTok(cur.cached), d: delta(cur.cached, prev.cached), color: theme.subtle },
  ];
  const cardsRow2 = [
    { label: '活跃时长', value: fmtHrs(cur.active), d: delta(cur.active, prev.active), color: theme.accent },
    { label: '总时长', value: fmtHrs(cur.duration), d: delta(cur.duration, prev.duration), color: theme.text },
    { label: '会话数', value: fmtInt(cur.sessions), d: delta(cur.sessions, prev.sessions), color: theme.text },
    { label: '总消息数', value: fmtInt(cur.msgs), d: delta(cur.msgs, prev.msgs), color: theme.text },
    { label: '用户消息数', value: fmtInt(cur.userMsgs), d: delta(cur.userMsgs, prev.userMsgs), color: theme.text },
  ];
  ...
  <defs>
    <linearGradient id="barGrad" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0%" stop-color="${theme.barTop}"/>
      <stop offset="100%" stop-color="${theme.barBottom}"/>
    </linearGradient>
  </defs>
  <rect width="${W}" height="${H}" rx="14" fill="${theme.bg}" stroke="${theme.border}"/>
  <g font-family="'JetBrains Mono','SF Mono','PingFang SC','Menlo',monospace" text-rendering="geometricPrecision">
    ...
    <text x="${W - PAD - 4}" y="${H - 6}" font-size="9" fill="${theme.faint}" text-anchor="end">更新于 ${stamp} UTC+8 · vibecafe.ai/usage</text>
  </g>
</svg>
`;
}
